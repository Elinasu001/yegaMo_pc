...
<c:set var="complete" value="/${dsrId }/loan/04_completed" />
...
function next() {
	...
	if(!$('#productdetail').prop('checked')){
		customAlert({title : '안내', message : "가계대출 상품설명서의 금리인하요구권 설명을 확인하세요."});	
		return false;
	}
...
function setLimitList(res){
	...
	$("#loan_list").empty();
	var html ='<h3 style="color:#ec008c;margin:20px 10px 10px 0px; padding-left:10px;">신용대출</h3><table>';
	...
	for(var i=0; i<tempList.length;i++){
		if(tempList[i].PROD_TYPE=='Y'){
			...
			creditList +='<tr><th class="ta-c">상품명</th><td>'+tempList[i].PROD_NM2+'</td><td rowspan="3" class="chk_td"><label for="credit_check'+i+'" class="check"><input type="checkbox" name="credit_check" id="credit_check'+i+'" value="'+tempList[i].PROD_CD+'"></label></td></tr>';
			creditList +='<tr><th class="ta-c">한도</th><td>'+UtilFunc.gridFormatterThousandNumber((parseInt(tempList[i].APPR_AMT)/10000).toString())+' 만원</td></tr>';
			creditList +='<tr><th class="ta-c">금리</th><td>'+tempList[i].APPR_RT+'%</td></tr>';
			
		}else{
			pubList +='<tr><th class="ta-c">상품명</th><td>'+tempList[i].PROD_NM2+'</td><td rowspan="3" class="chk_td"><label for="pub_check'+i+'" class="check"><input type="checkbox" name="pub_check" id="pub_check'+i+'" value="'+tempList[i].PROD_CD+'"></label></td></tr>';
			pubList +='<tr><th class="ta-c">한도</th><td>'+UtilFunc.gridFormatterThousandNumber((parseInt(tempList[i].APPR_AMT)/10000).toString())+' 만원</td></tr>';
			pubList +='<tr><th class="ta-c">금리</th><td>'+tempList[i].APPR_RT+'%</td></tr>';
		}
	}
	
	html += creditList+'</table><h3 style="color:#ec008c;margin:20px 10px 10px 0px; padding-left:10px;">정책자금</h3><table>'+pubList+'</table>';
	$("#loan_list").html(html);
	$("#limit_result").show();
...
function termsRadioChangeEvent(na){
	 ...
	 if($("img[name=barImg]").css("width") =="500px"){
		...
		var dataTooltip = $(na).attr("data-tooltip"); //find(".ctm-check")
		var layerId;
		if(dataTooltip == "agreement1-layer"){
			...
			codeI = "T000800015";
		}else if(dataTooltip == "agreement18-layer"){
			layerId = "agreement18Layer";
		}else{
			layerId = '';
		}
		
		if(layerId != undefined && codeI !=''){
			if(layerId == "agreement18-layer"){
...
<input type="hidden" id="AGE_CHECK_YN" value="${AGE_CHECK_YN }"> 
...
<input type="hidden" name="jobCd" id = "jobCd" value="${jobCd}"/>
<input type="hidden" name="bizNo" id = "bizNo" value="${bizNo}"/>
</form>
<%-- 약관 체크를 위함 --%>
<input type="hidden" id="prod_cd" value="TERMSALL"/>
	        ...
	        <div class="sub-contents default-wrap">
	        	<div class="loan-wrap">
					...
					<form id="frm_terms">
						...
						<div class="line-box">『특정금융거래정보의 보고 및 이용 등에 관한 법률』, 『공중 등 협박목적 및  대량살상무기확산을 위한 자금조달행위의 금지에 관한 법률』에 따라 고객은 당사와 거래 시 고객확인 및 검증을 위하여 아래의 내용을 기재하셔야  합니다. 
									...
									고객이 제공하신 정보 또는 자료는  ‘금융실명거래 및 비밀보장에 관한 법률’과 '신용정보의 보호 및 이용에 관한 법률' 등 관련법령에 따라 보호됩니다.</div>
	                    <hr class="mt30" id="con0">
						<ul class="confirm-wrap mb30">
							<li class="question-wrap" id="con1">
								<p class="">개인거래목적</p>
								<p class="radio-area mt15 ta-ㅣ">
								<p></p>
									<span class="ctm-radio">
										<label for="indv_trn_prps_radio_1" class="radio" style="margin-right: 10px; margin-bottom: 7px;">급여 및 생활비<input type="radio" name="indv_trn_prps_radio" id="indv_trn_prps_radio_1" value="3010"></label>
										<label for="indv_trn_prps_radio_2" class="radio" style="margin-right: 10px; margin-bottom: 7px;">저축 및 투자<input type="radio" name="indv_trn_prps_radio" id="indv_trn_prps_radio_2" value="3020"></label>
										<label for="indv_trn_prps_radio_3" class="radio" style="margin-right: 10px; margin-bottom: 7px;">사업상 거래<input type="radio" name="indv_trn_prps_radio" id="indv_trn_prps_radio_3" value="3030"></label>
										<label for="indv_trn_prps_radio_4" class="radio" style="margin-right: 10px; margin-bottom: 7px;">부동산 매매<input type="radio" name="indv_trn_prps_radio" id="indv_trn_prps_radio_4" value="3040"></label>
										<label for="indv_trn_prps_radio_5" class="radio" style="margin-right: 10px; margin-bottom: 7px;">상속, 증여성 거래<input type="radio" name="indv_trn_prps_radio" id="indv_trn_prps_radio_5" value="3050"></label>
										<label for="indv_trn_prps_radio_6" class="radio" style="margin-right: 10px; margin-bottom: 7px;">타금융회사 상품가입<input type="radio" name="indv_trn_prps_radio" id="indv_trn_prps_radio_6" value="3060"></label>
										<label for="indv_trn_prps_radio_7" class="radio" style="margin-right: 10px; margin-bottom: 7px;">대출담보제공<input type="radio" name="indv_trn_prps_radio" id="indv_trn_prps_radio_7" value="3070"></label>
										<label for="indv_trn_prps_radio_8" class="radio" style="margin-right: 10px; margin-bottom: 7px;">대출보증<input type="radio" name="indv_trn_prps_radio" id="indv_trn_prps_radio_8" value="3080"></label>
										<label for="indv_trn_prps_radio_9" class="radio" style="margin-right: 10px; margin-bottom: 7px;">대출연장<input type="radio" name="indv_trn_prps_radio" id="indv_trn_prps_radio_9" value="3090"></label>
										<label for="indv_trn_prps_radio_10" class="radio" style="margin-right: 10px; margin-bottom: 7px; ">대출원리금상환<input type="radio" name="indv_trn_prps_radio" id="indv_trn_prps_radio_10" value="3090"></label>
										<label for="indv_trn_prps_radio_11" class="radio" style="margin-right: 10px; margin-bottom: 7px;">차입, 부채 상환<input type="radio" name="indv_trn_prps_radio" id="indv_trn_prps_radio_11" value="3110"></label>
										<label for="indv_trn_prps_radio_13" class="radio">결제<input type="radio" name="indv_trn_prps_radio" id="indv_trn_prps_radio_13" value="3120"></label>
										</br><label for="INDV_TRN_PRPS_APV_ONE_CNTN" class="apv-check"><input type="checkbox" name="INDV_TRN_PRPS_APV_ONE_CNTN" id="INDV_TRN_PRPS_APV_ONE_CNTN" value="">보험료납부</label>
										<label for="INDV_TRN_PRPS_APV_TWO_CNTN" class="apv-check"><input type="checkbox" name="INDV_TRN_PRPS_APV_TWO_CNTN" id="INDV_TRN_PRPS_APV_TWO_CNTN" value="">공과금납부</label>
										<label for=INDV_TRN_PRPS_APV_THR_CNTN class="apv-check"><input type="checkbox" name="INDV_TRN_PRPS_APV_THR_CNTN" id="INDV_TRN_PRPS_APV_THR_CNTN" value="">카드대금</label>
									</span>
								</p>
							</li>
							<li class="question-wrap" id="con1">
								<p class="">거래자금의 원천 및 출처</p>
								<p class="radio-area mt15 ta-l">
								<p></p>
									<span class="ctm-radio">
										<label for="indv_trn_cpt_radio_1" class="radio" style="margin-right: 10px; margin-bottom: 7px;">근로 및 연금소득<input type="radio" name="indv_trn_cpt_radio" id="indv_trn_cpt_radio_1" value="7010"></label>
										<label for="indv_trn_cpt_radio_2" class="radio" style="margin-right: 10px; margin-bottom: 7px;">퇴직소득<input type="radio" name="indv_trn_cpt_radio" id="indv_trn_cpt_radio_2" value="7020"></label>
										<label for="indv_trn_cpt_radio_3" class="radio" style="margin-right: 10px; margin-bottom: 7px;">사업소득<input type="radio" name="indv_trn_cpt_radio" id="indv_trn_cpt_radio_3" value="7030"></label>
										<label for="indv_trn_cpt_radio_4" class="radio" style="margin-right: 10px; margin-bottom: 7px;">부동산 임대소득<input type="radio" name="indv_trn_cpt_radio" id="indv_trn_cpt_radio_4" value="7040"></label>
										<label for="indv_trn_cpt_radio_5" class="radio" style="margin-right: 10px; margin-bottom: 7px;">부동산 양도소득<input type="radio" name="indv_trn_cpt_radio" id="indv_trn_cpt_radio_5" value="7050"></label>
										<label for="indv_trn_cpt_radio_6" class="radio" style="margin-right: 10px; margin-bottom: 7px;">금융소득(이자 및 배당)<input type="radio" name="indv_trn_cpt_radio" id="indv_trn_cpt_radio_6" value="7060"></label>
										<label for="indv_trn_cpt_radio_7" class="radio" style="margin-right: 10px; margin-bottom: 7px;">상속/증여<input type="radio" name="indv_trn_cpt_radio" id="indv_trn_cpt_radio_7" value="7070"></label>
										<label for="indv_trn_cpt_radio_8" class="radio">일시 재산양도로 인한 소득<input type="radio" name="indv_trn_cpt_radio" id="indv_trn_cpt_radio_8" value="7080"></label>
									</span>
								</p>
							</li>
						</ul>
...
<!-- 						<dd id="T000800004" style="margin-left: 0px !important;"> -->
<!-- 					        <label for="productdetail" class="check productdetail-default" onclick="checkDuty(this);" style="font-weight:bold;"> -->
<!-- 					        <input type="hidden" name="terms_agreement_detail" value="Y"/> -->
<!-- 					        <input type="checkbox" id="productdetail" value=""> -->
<!-- 					        <span>(필수)가계대출 상품설명서 </span> -->
...
<!-- 					     </dd> -->
				     ...
				     <dl class ="agree-wrap allChk mt30">
							<dt>
							...
							<dd id="T000800001">
								<label for="agree-check1" class="check">
									<input type="checkbox" id="agree-check1" name="terms_agreement_1" value="">
									<span>(필수)개인(신용)정보 수집·이용·제공·조회 동의서</span>
							...
							<dd id="T000800006">
								<label for="agree-check2" class="check">
									...
									<span>(필수)개인(신용)정보 조회동의서(서민금융진흥원用)</span>
							...
							<dd id="T000800007">
								<label for="agree-check3" class="check">
									...
									<span>(필수)개인(신용)정보 수집 ·이용 ·제공 동의서(서민금융진흥원用)</span>
									<button class="btn-normal tiny" data-tooltip="manual011-layer">자세히보기</button>
								</label>
							</dd>
							...
							<!-- <dd id="T000800008">
								<label for="agree-check4" class="check">
									...
									<span>(필수)개인(신용)정보 및 기업(신용)정보 조회 ·수집 ·이용·제공과 행정정보공동이용, 전자금융거래확인서 발급을 위한 동의서</span>
									<button class="btn-normal tiny" data-tooltip="manual011-layer">자세히보기</button>
								</label>
							</dd> -->
							...
							<!-- <dd id="T000800004">
								<input type="hidden" name="terms_agreement_detail" value="Y"/>
						        <label for="productdetail" class="check productdetail-default">
						        <input type="checkbox" id="productdetail" value="">
						        <span>(필수)가계대출 상품설명서 </span>
					            <button class="btn-normal tiny detail-layer-popup" data-tooltip="manual011-layer" title="상품설명서 확인하기">자세히보기</button>
						        </label>
							...
							<dd id="T000800010">
								...
								<span>(필수)CMS 출금이체 동의</span>
								<button class="btn-normal tiny" data-tooltip="manual011-layer">자세히보기</button>
								</label>
							</dd>
							<dd id="T000800018">
								<label for="agree-check18" class="check">
								<input type="checkbox" name="terms_agreement_18" id="agree-check18" value="">
								<span>(필수)개인(신용)정보 제3자 제공 동의서 [신용대출, 담보대출, 정책자금대출]</span>
								<button class="btn-normal tiny" data-tooltip="manual011-layer">자세히보기</button>
								</label>
							</dd>
							<!-- //180612 약관추가 -->
						...
						<dl class="agree-wrap mdChk mt30">
                             <dt>
                             	<label id="all-md-check-lb" for="all-md-check" class="check" style="font-weight:bold;">
                             		<input type="checkbox" id="all-md-check" value="" >다음의 공공 마이데이터 필수약관에 모두 동의합니다.
							...
							<dd id="T001700001" >
								<label for="agree-check44" class="check"><input type="checkbox" name="terms_agreement_44" class="agree-check44" id="agree-check44" value="" >(필수)공공 마이데이터 개인(신용)정보 수집·이용·제공 동의서
							...
							<dd id="T001700002"  >
								<label for="agree-check45" class="check"><input type="checkbox" name="terms_agreement_45" class="agree-check45" id="agree-check45"  value="" >(필수)본인 행정정보 제3자 제공 요구서
									<button type="button" name="button" class="btn-normal tiny agree-wrap-btn" data-tooltip="manual011-layer" title="공공 마이데이터 개인(신용)정보 수집·이용·제공 동의서" >자세히보기</button>
								</label>
							</dd>
							<dd id="T001700003"  >
								<label for="agree-check47" class="check"><input type="checkbox" name="terms_agreement_47" class="agree-check47" id="agree-check47"  value="" >(필수)본인정보 제3자 제공 요구서
									<button type="button" name="button" class="btn-normal tiny agree-wrap-btn" data-tooltip="manual011-layer" title="공공 마이데이터 개인(신용)정보 수집·이용·제공 동의서" >자세히보기</button>
								</label>
							</dd>
							<dd>
						...
						<dl class="agree-wrap mt30 chChk">
							<dt>
	                        	<label for="all-ch-check" class="check">
	                        		<input type="checkbox" name="all-ch-check" id="all-ch-check" value="">다음의 선택약관에 모두 동의합니다.
									<a class="btn-normal tiny hide" data-tooltip="agreementChall-layer">자세히보기</a>
								</label>
	                        </dt>
	                        <br>
							<dd id="T000800003">
							...
							<!-- <dd>
								<label for="agree-check10" class="check">
									<input type="checkbox" name="terms_agreement_10" id="agree-check10" value="" >
									<span>(선택) 개인(신용)정보 선택적 동의서(통신정보 활용 동의)</span>
									<a href="#" class="btn-normal tiny" data-tooltip="agreement10-layer">자세히보기</a>
								</label>
								<div class="subcheck-area" id="agree10_sub">
	                               	<div class="sub-box">
	                          			<div><span class="check-tit subbox-all" style="font-weight:normal !important;">개인(신용)정보 수집ㆍ이용 동의여부</span></div>
	                          			<div><span class="check-tit subbox-all" style="font-weight:normal !important;">고유식별정보 수집ㆍ이용 동의여부</span></div>
	                          			<div><span class="check-tit subbox-all" style="font-weight:normal !important;">개인(신용)정보 수집 제공ㆍ조회 동의여부</span></div>
	                          			<div><span class="check-tit subbox-all" style="font-weight:normal !important;">고유식별정보 제공ㆍ조회 동의여부</span></div>
	                          			<div><span class="check-tit subbox-all" style="font-weight:normal !important;">개인(신용)정보 제공 동의여부</span></div>
	                          			<div><span class="check-tit subbox-all" style="font-weight:normal !important;">개인(신용)정보 처리위탁 동의여부</span></div>
	                                </div>
	                             </div>
							</dd> -->
							<dd id="T000800011">
							...
							<dd id="T000800016">
								<label for="agree-check16" class="check">
									<input type="checkbox" name="terms_agreement_16" id="agree-check16" value="">
									<span>(선택)개인(신용)정보 수집·이용 동의서(보증료 우대 서비스)</span>
									<button class="btn-normal tiny" data-tooltip="manual011-layer" onclick="javascript:termsRadioChangeEvent(this);">자세히보기</button>
								</label>
							...
							<dd id="T000800017">
								<label for="agree-check17" class="check">
									<input type="checkbox" name="terms_agreement_17" id="agree-check17" value="">
									<span>(선택)개인(신용)정보 수집·이용 동의서(상품서비스 안내 등) </span>
									<button class="btn-normal tiny" data-tooltip="manual011-layer" onclick="javascript:termsRadioChangeEvent(this);">자세히보기</button>
								</label>
							...
							<dd id="T001000002">
                            	<label for="agree-check46" class="check"><input type="checkbox" id="agree-check46" value="">(선택)개인(신용)정보 수집·이용·제공·조회 동의서 [멤버십 회원가입 동의]
                          	...
                          	<dd id="T001000001" onclick="agrchkk();">             
								<label for="agree-check46_1" class="check"><input type="checkbox" id="agree-check46_1" value="" >예가람저축은행 멤버쉽 회원 약관
...
<!-- 					<br>일부 카드의 경우 해당 카드사의 정보미제공으로 본인인증이 불가 합니다.(신한, 현대카드) -->
					...
					<ul class="auth-type" style="text-align: center;">
					...
					<div id="limit_result" style="display:none;">
						<h3 class="tit mt40">한도조회 결과</h3>
						<div id="loan_list" class="mt30">
						</div>
						<div class="btn-wrap" style="margin-top:30px;">
							<span><button class="btn-normal pink" onclick="req_next();">신청</button></span>
						</div>
					</div>
...
<!--         <div id="limitLayer"></div> -->
		...
		<!-- //e:전체 레이어팝업 -->
		 ...
		 <!-- s:전체 레이어팝업 -->
<!-- 		<div id="mdagreementallLayer" class="layerpopup agreementall-layer pdf-alllayer" tabindex="0" data-tooltip-con="mdagreementall-layer" data-focus="mdagreementall-layer" data-focus-prev="btn-menu-close" style = "left: 0px;right: 0px;max-height: 695px;overflow-y: auto;top: -80.5px;"> -->
		<div id="mdagreementallLayer" class="layerpopup agreementall-layer pdf-alllayer" tabindex="0" data-tooltip-con="mdagreementall-layer" data-focus="mdagreementall-layer" data-focus-prev="btn-menu-close" style = "left: 0px;right: 0px;max-height: 695px;overflow-y: auto;top: -80.5px;">
			<div class="pop-wrap">
				<div class="all-agree">*공공마이데이터 필수 약관 전체 보기 ( <span class="view2"></span> / <span class="total2"></span> )</div>
				<iframe id="mdallViewer" src="" style="width: 100%;height: 450px;margin-top:-27px;"></iframe>
...
<!-- 				</div> -->
		...
		<script>
			$(function(){
				//20211203 약관 PDF 적용 
				$(".sub-contents").find(".agree-wrap").find("button").on("click", function (){
						...
						streamdocs.document.open({ 
							...
							streamdocs.document.getPageCount().then((result) => {
								...
								streamdocs.addEventListener('currentPageIndexChange', (event) => { 
									...
									if( (event.data.currentPageIndex+1) == cnt-1 ){
