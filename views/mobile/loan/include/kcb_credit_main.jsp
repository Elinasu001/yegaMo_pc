...
<%-- 페이징 완료페이지 --%>
<c:set var="KcbOffCreditAjax" value="/${dsrId }/loan/ajaxKcbOffCredit"/>
<!DOCTYPE html>
<html lang="ko">
<head>
<link href="<c:url value="/resources/mobile/css/layout.css"/>" rel="stylesheet" />
<link href="<c:url value="/resources/mobile/css/jquery-ui.css"/>" rel="stylesheet" /><!-- 180424 추가 -->
<script src="<c:url value="/resources/mobile/js/jquery-1.11.3.min.js"/>" type="text/javascript"></script>
<script src="<c:url value="/resources/mobile/js/jquery-ui.js"/>" type="text/javascript"></script><!-- 180424 추가 -->
<script src="<c:url value="/resources/mobile/js/default.js"/>" type="text/javascript"></script>
<script src="<c:url value="/resources/mobile/js/ui.js?v=20190624"/>" type="text/javascript"></script>
...
<form name="form1">
	...
	<input type="hidden" name="mdlTkn">						<!-- 토큰 --> 
	<!-- 필수 항목 //-->	
	</form>
<script>
	var step= "${step}";
...
// 	});
	...
	function OkCert3_setResult(result) {	// AJAX 통신이 성공 처리된 경우 호출되는 함수
		...
		if (responseJson.RSLT_CD == "T300") { 	// 거래 성공인 경우
			document.getElementsByName("mdlTkn")[0].value = responseJson.MDL_TKN;
